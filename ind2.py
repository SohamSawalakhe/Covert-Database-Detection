...
class CovertDBDetectionApp:
    def __init__(self, root):
        ...
        self.performance_folder = None  # For performance analysis
        self.create_ui()
    ...
    def scan_hidden_db_directory(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for root_dir, dirs, files in os.walk(directory):
                for file in files:
                    ...
                    future = executor.submit(self.advanced_detection, file_path, method)
        ...
        if found_files:
            for f in found_files:
                self.detected_tree.insert("", "end", values=(f, method))
            self.detect_output_text.insert(tk.END, f"Found {len(found_files)} hidden file(s) using {method} in {elapsed:.2f} seconds.\n")
        else:
            self.detect_output_text.insert(tk.END, f"No hidden files found using {method} in {elapsed:.2f} seconds.\n")
    ...
    def advanced_detection(self, file_path, method):
        try:
            if method == "Steganography":
                ...
                return self.deep_learning_detection(file_path)
        except Exception:
            return False
        return False

    def deep_learning_detection(self, file_path):
        try:
            with open(file_path, "rb") as f:
                data = f.read(1024)
            if not data:
    ...
    def create_performance_analysis_page(self):
        ...
        select_btn = tk.Button(btn_frame, text="Select Folder", command=self.select_performance_folder)
        select_btn.pack(side=tk.LEFT, padx=5)
        run_btn = tk.Button(btn_frame, text="Run Analysis", command=self.run_performance_analysis)
        run_btn.pack(side=tk.LEFT, padx=5)
        
        self.perf_status_label = tk.Label(frame, text="No analysis run yet.", font=("Arial", 10))
        self.perf_status_label.pack(pady=5)
        return frame

    def select_performance_folder(self):
        folder = filedialog.askdirectory(initialdir="/", title="Select Folder for Performance Analysis")
        if folder:
            self.performance_folder = folder
            self.perf_status_label.config(text=f"Selected folder: {folder}")

    def run_performance_analysis(self):
        if not self.performance_folder:
            messagebox.showerror("Error", "Please select a folder for performance analysis.")
            return
        methods = ["Rename Extension", "Steganography", "Cryptography", "File Chunk Split",
                   "Alternate Data Streams (ADS)", "Machine Learning Obfuscation", "Hybrid Method", "Deep Learning"]
        performance_data = {}
        for method in methods:
            t, count = self.measure_detection_time(method, self.performance_folder)
